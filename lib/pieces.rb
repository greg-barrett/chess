...
class Pieces
  attr_accessor :current_location, :name, :first_location, :last_location, :en_passant, :character, :colour
...
class King<Pieces
  attr_accessor :moved, :castle, :in_check, :in_checkmate
  @@kings=0
  def initialize
    @@kings+=1
    self.set_starting_location
  ...
  def castling?(board, all_pieces, new_location)
    ...
    if self.never_moved? == true && self.in_check == false && self.relevant_sqs_empty?(board, new_location) == true && self.sqs_not_under_attack?(all_pieces, new_location, board) == true  && self.rook_moved?(all_pieces, new_location) == false
        @castle<<new_location
        @castle
    end
  end
  ...
  def checkmate(all_pieces, current_player, board)
    if self.in_check==true
      ...
      if self.calculated_moves.length==0
        ...
        return self.in_checkmate=false
      end
    else
...
def check?(all_pieces, current_player, board)
  if current_player.colour=="White"
    all_pieces.each do |piece|
      if piece.colour =="Black"
        piece.calculate_moves(board)
        piece.calculated_moves.each do |pos|
          if self.current_location == pos
            return self.in_check=true
          else
              self.in_check=false
          end
        end
  ...
  else #current_player.piece.colour =="Black"
    all_pieces.each do |piece|
